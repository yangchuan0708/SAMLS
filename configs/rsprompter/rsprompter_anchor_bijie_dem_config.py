...
model_cfg = dict(
    ...
    backbone1=dict(type='unet', in_channels=1,
                 out_channels=1),
    panoptic_head=dict(
...
callbacks = [
    dict(
        ...
        show=False,
        wait_time=1.,
        test_out_dir=f'results/{task_name}/{exp_name}/visualization',
    )
]
...
data_parent = r'D:/RSPrompter-cky/data/bijie_hillshade'
train_data_prefix = 'train2017/'
val_data_prefix = 'test2017/'
test_data_prefix = 'val2017/'
...
test_loader = dict(
        ...
        dataset=dict(
            ...
            # ann_file='annotations/SSDD_instances_val.json',
            # data_prefix=dict(img_path='imgs'),
            ann_file='annotations/bijie_val.json',
            data_prefix=dict(img_path=test_data_prefix + '/image'),
            test_mode=True,
            filter_cfg=dict(filter_empty_gt=True, min_size=32),
            pipeline=test_pipeline,
            backend_args=backend_args))
