...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.pos_embed is not None: # 1,64,64,768
            ...
            demx = demx + pos_embed.repeat(demx.shape[0], 1, 1, 1) #2,32,32,768
...
class ParaBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, demx: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        shortcut = x
        x = self.norm1(x)
        ...
        if self.window_size == 0:
            sax = self.Space_Adapter(x, demx, demx)  # b h w c
            x = x + sax
            demx = self.refine_Adapter(demx.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
