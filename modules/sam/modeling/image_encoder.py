...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        #     kernel_size=(patch_size, patch_size), #16
        #     stride=(patch_size, patch_size), #16
        #     in_chans=in_chans,
        #     embed_dim=embed_dim,
        # )
        ...
        if use_abs_pos:
            ...
            # self.post_pos_embed1 = PostPosEmbed(embed_dim=embed_dim, ori_feature_size=1024 // 16,
            #                                    new_feature_size=img_size // patch_size)  # new to sam
        ...
        #         # window_size=0,
        #         input_size=(1024 // 16, 1024 // 16),
        #         # input_size=(img_size // patch_size, img_size // patch_size),
        #     )
        #     self.cnnblocks.append(block)
        ...
        # self.input_Adapter1 = Adapter(embed_dim)
        # self.input_Adapter = Adapter(embed_dim)

    def forward(self, x: torch.Tensor) -> torch.Tensor:

        # cnnx = self.patch_embed1(x[:, 3:, :, :])
        # cnnx = self.input_Adapter1(cnnx)###

        cnnx = self.cnn_embed(x[:,3:,:,:])  # b h w c
        # cnnx = self.cnn_embed(x[:, :3, :, :])  # b h w c 针对rgb
        #
        x = self.patch_embed(x[:, :3, :, :]) # 2,32,32,768    (2,64,64,768)
        # x = self.patch_embed(x[:, 3:, :, :])  # 2,32,32,768    (2,64,64,768)
        # x = self.input_Adapter(x)###

        if self.pos_embed is not None: # 1,64,64,768
            # x = x + self.pos_embed
            # cnnx = cnnx + self.pos_embed
            pos_embed = self.post_pos_embed(self.pos_embed) # 1 h w c      1,32,32,768
            x = x + pos_embed.repeat(x.shape[0], 1, 1, 1) #2,32,32,768
            # pos_embed1 = self.post_pos_embed1(self.pos_embed)
            # cnnx = cnnx + pos_embed.repeat(cnnx.shape[0], 1, 1, 1) #2,32,32,768

        inter_features = []

        # for blk1 in self.cnnblocks:
        #     cnnx = blk1(cnnx)
...
class ParaBlock(nn.Module):
    ...
    def __init__(
        ...
        # self.MLP_Adapter1 = Adapter(dim, skip_connect=False)
        # self.MLP_Adapter2 = Adapter(dim)  # with skip connection
        # self.MLP_Adapter3 = Adapter(dim)  # with skip connection
        ...
        if self.window_size == 0:
            ...
            #     # input_size=(img_size // patch_size, img_size // patch_size),
            # )
            # self.MLP_Adapter1 = Adapter(dim, skip_connect=False)  # new to sam, MLP-adapter, no skip connection
            # self.MLP_Adapter2 = Adapter(dim)  # with skip connection
            # self.MLP_Adapter3 = Adapter(dim)  # with skip connection
            self.scale = 0.5
        # ---------------------------------------------------
    ...
    def forward(self, x: torch.Tensor, cnnx: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # shortcut1 = cnnx
        # cnnx = self.norm1(cnnx)

        # Window partition
        if self.window_size > 0:
            H, W = x.shape[1], x.shape[2]
            x, pad_hw = window_partition(x, self.window_size) #18,14,14,768
            # cnnx, pad_hw = window_partition(cnnx, self.window_size)  # 18,14,14,768

        if self.window_size == 0:
            sax = self.Space_Adapter(x, cnnx, cnnx)  # b h w c
            x = x + sax
            cnnx = self.refine_Adapter(cnnx.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            # cnnx = self.refine_Adapter(cnnx)

        x = self.attn(x)
        # cnnx = self.attn(cnnx)

        # if self.window_size == 0:
        #     x = self.MLP_Adapter2(x)
        #     cnnx = self.MLP_Adapter3(cnnx)

        # Reverse window partition
        if self.window_size > 0:
            x = window_unpartition(x, self.window_size, pad_hw, (H, W))
            # cnnx = window_unpartition(cnnx, self.window_size, pad_hw, (H, W))
        ...
        # cnnx = shortcut1 + cnnx
        # xn1 = self.norm2(cnnx)
        # cnnx = cnnx + self.mlp(xn1)

        if self.window_size == 0:
            x = x + self.scale * self.MLP_Adapter(xn)
            # x = x
            # cnnx = cnnx + self.scale * self.MLP_Adapter1(xn1)
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        # self.add_module('MLP_Adapter', Adapter(dim, skip_connect=False))
        # self.add_module('Space_Adapter', Adapter(dim))
        # self.scale = scale
        ###################################
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # xn = self.norm2(x)
        # x = x + self.mlp(self.norm2(x)) + self.scale * self.MLP_Adapter(xn)  ########################
...
def window_partition(x: torch.Tensor, window_size: int) -> Tuple[torch.Tensor, Tuple[int, int]]:
    ...
    x = x.view(B, Hp // window_size, window_size, Wp // window_size, window_size, C) #2,3,14,3,14,768
    windows = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, window_size, window_size, C)
    return windows, (Hp, Wp) #18,14,14,768
...
class PostPosEmbed(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        downtimes = int(math.log2(ori_feature_size//new_feature_size)) #1
...
class PatchEmbed0(nn.Module):
    ...
    def __init__(
        ...
        #     embed_dim, embed_dim, kernel_size=2, stride=(2, 2), padding=padding
        # )
        # self.proj2 = SingleDown(embed_dim, embed_dim)  ###
        ...
        # self.proj = nn.Conv2d(
        #     in_chans, embed_dim, kernel_size=16, stride=(8, 8), padding=(4, 4)
        # )
        # self.proj1 = nn.Conv2d(
        #     embed_dim, embed_dim, kernel_size=4, stride=(4, 4), padding=(0, 0)
        # )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = F.interpolate(x, (256+8, 256+8), mode="bilinear", align_corners=False)
        # x = F.interpolate(x, (256 + 8, 256 + 8), mode="nearest", align_corners=False)
        # x = self.proj(x) # 32
        x = self.proj(x)
        # x = self.proj2(x)
        # B C H W -> B H W C
...
class SingleCNNEmbed(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        # downtimes = int(math.log2(32)) #32
        downtimes = int(math.log2(patchsize)) #8
        mid_channel = 64
        # mid_channel = 32
